...
document.querySelector('addNumbers').addEventListener('click', additionNumber);

// Step 6: Using function expressions, repeat Steps 1-5 with new functions named subtract and subtractNumbers and HTML form controls with IDs of minuend, subtrahend, difference and subtractNumbers
const subtractNumbers = function() {
    let minuend = parseInt(document.querySelector('#minuend').value);
    let subtrahend = parseInt(document.querySelector('#subtrahead').value);
    let result = subtract(minued, subtrahend);
...
const multiply = (number1, number2) => number * number2;
...
document.querySelector('#sumOfMulitplied').textContent = numbers.map(number => number * 2).reduce((sum, number) => sum + number, 0);